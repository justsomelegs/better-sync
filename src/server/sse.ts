...
export function createSseStream(config?: SseConfig) {
	...
	const KEEPALIVE_BYTES = textEncoder.encode(':keepalive\n\n');
	const RECOVER_BYTES = textEncoder.encode('event: recover\ndata: {}\n\n');
			...
			start(controller) {
				...
				if (opts.lastEventId) {
					...
					if (idx >= 0) {
						...
						controller.enqueue(RECOVER_BYTES);
					}
				}
				send = (frame: Uint8Array) => controller.enqueue(frame);
				subscribers.add(send);
				timer = setInterval(() => controller.enqueue(KEEPALIVE_BYTES), config?.keepaliveMs ?? 15000);
				if (opts.signal) {
