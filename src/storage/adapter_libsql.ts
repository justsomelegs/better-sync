...
export function libsqlAdapter(_config: { url: string; authToken?: string }): DatabaseAdapter {
	// Temporary in-memory implementation; to be replaced with real libsql client
	const tables = new Map<string, Map<string, any>>();
	const ulid = monotonicFactory();
	return createAdapter({
		async ensureMeta() { /* no-op for in-memory */ },
