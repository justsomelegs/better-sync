...
function startServer() {
  const srv = betterSync({ basePath: "/api/sync", forbid: () => true });
...
describe("error mapping - forbidden", () => {
  ...
  it("returns SYNC:FORBIDDEN for forbidden requests", async () => {
    const res = await fetch(`${url}/api/sync/pull?model=todo`);
    expect(res.status).toBe(403);
    const data = await res.json();
    expect(data.ok).toBe(false);
    expect(data.error.code).toBe("SYNC:FORBIDDEN");
  });
});
