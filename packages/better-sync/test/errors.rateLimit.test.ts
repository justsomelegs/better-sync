...
function startServer() {
	const srv = betterSync({ basePath: "/api/sync", shouldRateLimit: () => true });
...
describe("error mapping - rate limit", () => {
	...
	it("returns SYNC:RATE_LIMITED when rate limit applies", async () => {
		const res = await fetch(`${url}/api/sync/pull?model=todo`);
		expect(res.status).toBe(429);
		const data = await res.json();
		expect(data.ok).toBe(false);
		expect(data.error.code).toBe("SYNC:RATE_LIMITED");
	});
});
