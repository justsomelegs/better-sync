...
function tableName(name) { return name; }
function ensureUserTable(db, table) {
...
function bumpVersion(db, table, pk) {
    const sel = db.prepare(`SELECT seq FROM _sync_seq WHERE table_name=?`).get(table);
    const next = (sel?.seq ?? 0) + 1;
    if (sel)
        db.prepare(`UPDATE _sync_seq SET seq=? WHERE table_name=?`).run(next, table);
    else
        db.prepare(`INSERT INTO _sync_seq(table_name, seq) VALUES(?,?)`).run(table, next);
...
function getVersion(db, table, pk) {
    const row = db.prepare(`SELECT version FROM _sync_versions WHERE table_name=? AND pk=?`).get(table, pk);
    return row?.version ?? null;
}
function encodeCursor(table, orderBy, last) {
    if (!last)
        return null;
    const obj = { table, orderBy: orderBy ?? { updatedAt: 'desc' }, last };
...
export function sqliteAdapter(options) {
    ...
    return {
        ...
        async insert(table, row) {
            ...
            const merged = { ...row, id, updatedAt, version };
            ...
            try {
                ...
                if (String(e?.message ?? '').includes('UNIQUE'))
                    throw new Error('CONFLICT: unique constraint');
                throw e;
            }
            return merged;
        },
        async updateByPk(table, pkInput, set, opts) {
            ...
            const existing = db.prepare(`SELECT data FROM ${tableName(table)} WHERE id=?`).get(pk);
            if (!existing)
                throw new Error('NOT_FOUND: row not found');
        ...
        async selectByPk(table, pkInput, select) {
            ensureUserTable(db, table);
            const pk = canonicalPk(pkInput);
            const row = db.prepare(`SELECT data FROM ${tableName(table)} WHERE id=?`).get(pk);
            if (!row)
                return null;
